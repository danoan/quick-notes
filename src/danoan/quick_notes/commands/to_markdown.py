...
def parse(quick_note_table: model.QuickNoteTable) -> str:
    ...
    """
    env = Environment(loader=PackageLoader("danoan.quick_notes", package_path="templates"))

    template = env.get_template(
        Path("quick-note-table").joinpath("quick-note-table.tpl.md").expanduser().as_posix()
    )
